...
if (class_exists('Woocommerce')) {
	...
	function new_loop_shop_per_page( $cols ) {
		$num = dotted_get_option('per_shop') ? dotted_get_option('per_shop') : 6;
	...
	add_filter( 'woocommerce_output_related_products_args', 'dotted_related_products_args' );
	function dotted_related_products_args( $args ) {
	...
	add_filter( 'woocommerce_add_to_cart_fragments', 'dotted_header_add_to_cart_fragment' );
	function dotted_header_add_to_cart_fragment( $fragments ) {
		ob_start();
