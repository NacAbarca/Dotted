...
class dotted_Flickr_Widget extends WP_Widget {



	function dotted_Flickr_Widget() {
		...
		parent::__construct(false, $name = 'Dotted Flickr', array(

			'classname' => 'widget_flickr',

			'description' => esc_html__('Displays your latest Flickr feed.', 'dotted')
	...
	function widget($args, $instance) {
		...
		$fli_name = empty($instance['title']) ? esc_html__('Flickr', 'dotted') : apply_filters('widget_title', $instance['title']);
	...
	function form($instance) {
		...
		?>
			...
			<p>
				...
				<label for="<?php echo htmlspecialchars_decode($this->get_field_id('title')); ?>"><?php  esc_html_e('Flickr Name','dotted'); ?>:
			...
			<p>
				...
				<label for="<?php echo htmlspecialchars_decode($this->get_field_id('type')); ?>"><?php esc_html_e('Flickr Type:', 'dotted'); ?></label>
				...
				<select id="<?php echo htmlspecialchars_decode($this->get_field_id('type')); ?>" name="<?php echo htmlspecialchars_decode($this->get_field_name('type')); ?>">
					...
					<option<?php if($fli_type == 'user') { echo " selected='selected'"; } ?> name="<?php echo htmlspecialchars_decode($this->get_field_name('type')); ?>" value="user"><?php esc_html_e('user', 'dotted'); ?></option>



					<option<?php if($fli_type == 'group') { echo " selected='selected'"; } ?> name="<?php echo htmlspecialchars_decode($this->get_field_name('type')); ?>" value="group"><?php esc_html_e('group', 'dotted'); ?></option>
			...
			<p>
				...
				<label for="<?php echo htmlspecialchars_decode($this->get_field_id('id')); ?>"><?php  esc_html_e('Flickr ID','dotted'); ?>(<a target="_blank" href="http://www.idgettr.com">idGettr</a> ex: 71865026@N00):
			...
			<p>
				...
				<label for="<?php echo htmlspecialchars_decode($this->get_field_id('number')); ?>"><?php esc_html_e('Number of photos:','dotted'); ?>
...
register_widget('dotted_Flickr_Widget');